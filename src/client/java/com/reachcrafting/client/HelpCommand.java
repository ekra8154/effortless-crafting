...
public final class HelpCommand {
	...
	static int runTopic(String rawTopic) {
		...
		return switch (topic) {
			...
			case "general" -> page(new Object[]{
				...
				"§7Bigger:§r Alt + click the result arrow latches bulk (no cap, repeated max crafts).",
			...
			case "retrievethencraft" -> page(new String[]{
				...
				"§7Copies already in a chest?§r A Ctrl-assisted click on a recipe whose output sits in nearby storage (the green dot under the craftable dot) pulls those first instead of crafting duplicates. Plain clicks are untouched.",
	...
	static int runTips() {
		return page(new String[]{
			...
			"§7Alt + click the result slot§r latches bulk mode; the orange outline around the arrow shows it.",
