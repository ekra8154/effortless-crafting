...
public final class HelpCommand {
	...
	static int runTopic(String rawTopic) {
		...
		return switch (topic) {
			...
			case "nearby" -> page(new String[]{
				...
				"§7The dots§r on a recipe say what a Ctrl click would do: a filled dot crafts from your inventory alone, a plus shape will open chests. Yellow is a direct craft, orange a chain craft.",
	...
	static void tickPendingSettings(Minecraft client) {
		...
		if (FabricLoader.getInstance().isModLoaded("cloth-config")) {
			client.setScreenAndShow(com.reachcrafting.client.compat.ReachCraftingConfigScreen.create(null));
		}
	}
