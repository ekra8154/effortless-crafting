...
final class GridTopUp {
	 ...
	 */
	static boolean isRingForRecipe(Minecraft client, LocalPlayer player, ResourceLocation recipeId, RecipeCollection collection) {
	...
	private static RecipeIngredientSummary resolveSummary(Minecraft client, ResourceLocation recipeId, RecipeCollection collection) {
		...
		for (RecipeHolder<?> recipe : collection.getRecipes()) {
			if (recipe.id().equals(recipeId)) {
				return RecipeIngredientSummary.fromRecipe(recipe, gridSlotCount);
			}
		}
		return null;
	}
	...
	static boolean tryStageInsteadOfPlace(Minecraft client, LocalPlayer player, ResourceLocation recipeId, RecipeCollection collection) {
