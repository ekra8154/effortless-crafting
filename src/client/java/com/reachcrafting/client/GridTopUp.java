...
final class GridTopUp {
	...
	private static boolean spreadIntoGroupSlots(
		...
		client.gameMode.handleContainerInput(containerId, -999, 0, ContainerInput.QUICK_CRAFT, client.player);
		recordClick();
		for (int target : targets) {
			client.gameMode.handleContainerInput(containerId, target, 1, ContainerInput.QUICK_CRAFT, client.player);
			recordClick();
		}
		client.gameMode.handleContainerInput(containerId, -999, 2, ContainerInput.QUICK_CRAFT, client.player);
		recordClick();
		if (!menu.getCarried().isEmpty()) {
	...
	private static void click(Minecraft client, AbstractContainerMenu menu, int slotIndex) {
		client.gameMode.handleContainerInput(menu.containerId, slotIndex, 0, ContainerInput.PICKUP, client.player);
		recordClick();
	}
