 ...
 * {@code Set<ResourceLocation>}. Render-time queries are O(1) set lookups.</p>
 */
public final class ChainCraftabilityCache {
	 ...
	 */
	public static boolean isChainCraftable(ResourceLocation recipeId) {
		refreshIfNeeded(Minecraft.getInstance(), false);
		return recipeId != null && chainCraftableRecipeIds.contains(recipeId);
	}

	public static boolean isReachable(ResourceLocation recipeId) {
		refreshIfNeeded(Minecraft.getInstance(), false);
	...
	private static void refresh(Minecraft client, boolean fromTick, boolean synchronous) {
		...
		List<Recipe<?>> allRecipes = new java.util.ArrayList<>();
		...
		if (synchronous) {
			Computed computed = compute(allRecipes, gridSlotCount, registryAccess, indexStale, baseIndex, baseByOutput, counts);
	...
	private static Computed compute(
		...
		List<LightRecipe> localRecipeIndex = indexStale ? buildRecipeIndex(allRecipes, gridSlotCount, registryAccess) : baseIndex;
	...
	private static double reachDistance(Minecraft client) {
	...
	private static Classification classify(
		...
		Set<ResourceLocation> chainResult = new HashSet<>();
		Set<ResourceLocation> reachableResult = new HashSet<>();
		for (LightRecipe recipe : localRecipeIndex) {
			if (!allSlotsSatisfied(recipe.ingredientSlots, reachable)) {
				continue;
			}
	...
	private static VerifyResult verifyRecipe(LightRecipe recipe, int craftsNeeded, Map<String, Integer> state, Set<ResourceLocation> resolving, int depth, Map<String, List<LightRecipe>> localRecipesByOutput) {
	...
	private static VerifyResult fulfillRequirement(List<String> options, int needed, Map<String, Integer> state, Set<ResourceLocation> resolving, int depth, Map<String, List<LightRecipe>> localRecipesByOutput) {
		if (dfsOperations++ > 2000) return VerifyResult.NOT_CRAFTABLE;
	...
	private static List<LightRecipe> buildRecipeIndex(
		...
		for (Recipe<?> recipe : allRecipes) {
			...
			if (!fitsGrid(recipe, gridSlotCount)) {
				continue;
			}
			...
			index.add(new LightRecipe(outputId, outputCount, List.copyOf(slots), recipe.getId()));
		}
		return List.copyOf(index);
	}

	private static boolean fitsGrid(Recipe<?> recipe, int gridSlotCount) {
		if (gridSlotCount == 4) {
	...
	private record LightRecipe(String outputItemId, int outputCount, List<List<String>> ingredientSlots, ResourceLocation recipeId) {
	}
}
