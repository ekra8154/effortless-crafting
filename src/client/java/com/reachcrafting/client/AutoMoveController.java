...
final class AutoMoveController {
	...
	// 20 (not the modern 5): this pre-1.21.2 branch syncs the crafting grid one
	// packet at a time, so a partial grid transiently completes a DIFFERENT
	// recipe (one gold ingot alone previews the nuggets recipe) for longer than
	// the modern batch-drop trees. This restores the old-version grid-sync grace
	// that the branch's former resultMismatchGraceTicks provided.
	private static final int FOREIGN_RESULT_DEBOUNCE_TICKS = 20;
	...
	/**
	 * If the queued work has actually finished (result and cursor already empty), settle it as a
	 * success instead of letting a subsequent abort() discard the completion accounting. Credits any
	 * pending direct-eject output so the bulk summary stays accurate when the session is torn down.
	 */
	static void settleCompletedWork(Minecraft client) {
		...
		com.reachcrafting.ReachCraftingMod.diag(
			"[auto_move] settling completed work before abort pending={} organizing={} directEjectAwaitingSettlement={} credited={} target={} expected={}",
			pendingAutoMove,
			autoMoveOrganizing,
	...
	static void autoMoveResult(Minecraft client) {
		...
		// Chain final eject settlement, tolerant of both vanilla THROW
		// semantics. Older versions craft and drop ONE copy per result-slot
		// THROW and refill the slot immediately, so the slot is never
		// observed empty mid-batch: each craft is confirmed by the
		// staged-copy count dropping, and the confirming tick credits the
		// delta and fires the next throw. On versions where THROW drops the
		// whole staged batch, the grid empties in one step and the same
		...
		if (!autoMoveOrganizing) {
			if (resultSlot.hasItem() && resultSlot.mayPickup(client.player)) {
				...
				if (shouldEject) {
					...
					client.gameMode.handleInventoryMouseClick(menu.containerId, resultSlot.index, 1, ClickType.THROW, client.player);
					if (chainFinalResultEject) {
						// Depending on the version, THROW crafts-and-drops one
						// copy (with an immediate refill, so the slot is never
						// observed empty) or the whole staged batch. Credit is
						// confirmed by staged-grid depletion either way in the
						// chainEjectAwaitingRefresh block, which also fires
		...
		if (BulkAutoCraftController.shouldUseReservedOutputSweep(client, autoMoveTargetStack)) {
			...
			if (resultSlot.hasItem() && ItemStack.isSameItemSameComponents(resultSlot.getItem(), autoMoveTargetStack)) {
				com.reachcrafting.ReachCraftingMod.diag("[auto_move] reserved sweep observed result slot refill. Restarting loop.");
				autoMoveOrganizing = false;
				autoMoveWaitingTicks = 0;
				return;
			}
			...
			autoMoveTargetArrivalObserved = false;
			autoMoveTargetStack = ItemStack.EMPTY;
			com.reachcrafting.ReachCraftingMod.diag("[auto_move] reserved sweep complete: finishing batch");
			BulkAutoCraftController.onAutoMoveFinished(client, true);
			ChainCraftController.onAutoMoveFinished(client, true);
			return;
		}
