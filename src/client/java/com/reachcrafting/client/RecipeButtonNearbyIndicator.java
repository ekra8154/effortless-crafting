...
public final class RecipeButtonNearbyIndicator {
	...
	static String describe(Recipe<?> recipe, RecipeCollection collection) {
	...
	private static IndicatorState indicatorStateForRecipe(Recipe<?> recipe, RecipeCollection collection, ItemStack displayStack, boolean explicitVariantSelection) {
		if (recipe == null) {
		...
		if (ChainCraftabilityCache.isChainCraftableLocally(recipe.getId())) {
			return IndicatorState.CHAIN_LOCAL;
		}
		return ChainCraftabilityCache.isChainCraftable(recipe.getId()) ? IndicatorState.CHAIN_NEARBY : IndicatorState.NONE;
	}
	...
	public static boolean hasRetrievableOutput(Recipe<?> recipe, RecipeCollection collection, ItemStack displayStack, boolean explicitVariantSelection) {
		...
		Map<String, Integer> nearbyTotals = NearbyContainerCache.getReachableView(
			...
			(net.minecraft.client.Minecraft.getInstance().gameMode != null ? net.minecraft.client.Minecraft.getInstance().gameMode.getPickRange() : 4.5D)
		).aggregateCounts();
	...
	/**
	 * 1.20.1 computes nearby-craftability live; the only thing held is the
	...
	public static void renderButton(GuiGraphics guiGraphics, RecipeButton button) {
		...
		net.minecraft.resources.ResourceLocation stateKey = recipe.getId();
