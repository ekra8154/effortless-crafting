...
public final class BulkChainCraftController {
	...
	public record VariantFamily(
		net.minecraft.world.item.crafting.Recipe<?> clickedRecipe,
	...
	static void stop(boolean aborted, String reason) {
		...
		if (activeSession != null) {
			...
			// Degraded 1.20.1 summary: no crafted amounts (accounting is not
			// reliable on this version), just the item(s) and elapsed time. With
			// output variant switching the session names every variant it made.
			String crafted = activeSession.completedByItem().size() > 1
				? String.join(", ", activeSession.completedByItem().keySet().stream().map(ContainerUtils::getItemName).toList())
				: itemName;
			ReachCraftingModClient.sendBulkSummaryChat(
				"Bulk chain craft " + status + ": " + crafted + BulkDespawnWarning.elapsedSummarySuffix()
			);
		}
	...
	private static boolean trySwitchVariant(Minecraft client, Map<String, Integer> availableCounts, int batchTarget) {
		...
		java.util.Set<net.minecraft.resources.ResourceLocation> drained = new java.util.HashSet<>(session.drainedRecipeIds());
		...
		Map<net.minecraft.resources.ResourceLocation, Integer> totalScores = ChainVariantRanking.scoreVariants(candidates, availableCounts);
	...
	private static Map<String, Integer> collectAvailableCounts(Minecraft client, boolean allowNearby) {
		AvailableItemSnapshot availableItems = AvailableItemSnapshot.capture(client.player, client.screen);
	...
	private record BulkChainSession(
		...
		java.util.Set<net.minecraft.resources.ResourceLocation> drainedRecipeIds,
		...
		BulkChainSession withSelection(RecipeVariantResolver.Selection nextSelection, java.util.Set<net.minecraft.resources.ResourceLocation> drained) {
